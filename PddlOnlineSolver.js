...
const PATH = process.env.PAAS_PATH || '/package/forbiditerative-topk/solve';
...
async function parsePlan (json) {
    ...
    // HAHA
    else if (json.result.output['sas_plan.1'] && json.result.call.split(' ').includes('forbiditerative-topk') ) {
        
        console.log('Using parser forbiditerative-topk');
        let tmp = json.result.output['sas_plan.1'].split(';')[0].slice(0, -1).split('\n')
        tmp = tmp.map( line => line.toUpperCase().replace('(','').replace(')','').split(' '))
        tmp = tmp.filter(line => line != '');
        lines = tmp
    }
